...
let trees = [];
let clouds = [];
let highScore = 0; // Voeg deze regel toe voor de highscore
...
const scoreElement = createElementStyle(
  ...
  `Score: <span id="currentScore">0</span> Highscore: <span id="highScore">${highScore}</span>`
);
...
function resetGame() {
  ...
  restartButton.style.display = "none";
  scoreElement.innerHTML = `Score: <span id="currentScore">${score}</span> Highscore: <span id="highScore">${highScore}</span>`;
  platforms = [{ x: 50, w: 50 }];
